...
//        grantSmsPermission()
        ...
        PreferenceManager.getDefaultSharedPreferences(this).apply {
            this.getString(getString(R.string.telegram_chat_id_key), "0")
                ?.let { RetrofitManager.INSTANCE.updateTelegramChatID(it) }
            this.getString(getString(R.string.telegram_token_key), "0")
                ?.let { RetrofitManager.INSTANCE.updateTelegramToken(it) }
        }

        PreferenceManager.getDefaultSharedPreferences(this)
            .registerOnSharedPreferenceChangeListener { sharedPreferences, key ->
                when (key) {
                    getString(R.string.telegram_chat_id_key) -> {
                        sharedPreferences.getString(key, "0")
                            ?.let { RetrofitManager.INSTANCE.updateTelegramChatID(it) }
                    }

                    getString(R.string.telegram_token_key) -> {
                        sharedPreferences.getString(key, "0")
                            ?.let { RetrofitManager.INSTANCE.updateTelegramToken(it) }
                    }
                }
                LogUtil.i(this::class.java.simpleName, "onSharedPreferenceChanged: $key")
            }
        LogUtil.i(this::class.java.simpleName, "init: write preference")
    }
